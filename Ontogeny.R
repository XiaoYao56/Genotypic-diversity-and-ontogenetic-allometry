...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####anova:Ramet ~ Height × Diversity####
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~

anova.Ramet <- as.data.frame(anova(RametLmer))
anova.Ramet$Predictor <- row.names(anova.Ramet)

RametLmer.Anova.Table <- anova.Ramet %>%
  filter(Predictor != "(Intercept)") %>%
  mutate(
    Response  = "Ln(Ramet)",
    Predictor = plyr::mapvalues(
      Predictor,
      c("LogHeight",        "Diversity",           "LogHeight:Diversity"),
      c("Ln(Height)",       "Genotypic diversity", "Ln(Height) × Genotypic diversity")
    )
  ) %>%
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####~anova(): Inflorescence ~ Height × Diversity
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

anova.Inf.H <- as.data.frame(anova(InflorescenceLmer, type = 3))
anova.Inf.H$Predictor <- row.names(anova.Inf.H)

InflorescenceLmer.H.Anova.Table <- anova.Inf.H %>%
  filter(Predictor != "(Intercept)") %>%
  mutate(
    Response  = "Ln(Inflorescence)",
    Predictor = plyr::mapvalues(
      Predictor,
      c("LogHeight",        "Diversity",           "LogHeight:Diversity"),
      c("Ln(Height)",       "Genotypic diversity", "Ln(Height) × Genotypic diversity")
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
####~anova(): Inflorescence ~ Ramet × Diversity
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

anova.Inf.R <- as.data.frame(anova(InflorescenceLmer.Ramet, type = 3))
anova.Inf.R$Predictor <- row.names(anova.Inf.R)

InflorescenceLmer.R.Anova.Table <- anova.Inf.R %>%
  filter(Predictor != "(Intercept)") %>%
  mutate(
    Response  = "Ln(Inflorescence)",
    Predictor = plyr::mapvalues(
      Predictor,
      c("LogRamet",         "Diversity",           "LogRamet:Diversity"),
      c("Ln(Ramet)",        "Genotypic diversity", "Ln(Ramet) × Genotypic diversity")
    )
  ) %>%
  dplyr::select(Response,
                Predictor,
...
Allometry.LMM.anova <- bind_rows(
  RametLmer.Anova.Table,
  InflorescenceLmer.H.Anova.Table,
  InflorescenceLmer.R.Anova.Table
)

Allometry.LMM.anova


#write.xlsx(Allometry.LMM.anova,"Result/Table_Allometry_LMM_anova.xlsx")
...
# LMM：peak inflorescence ~ Diversity
PeakInflorescenceLmer <- lmer(Inflorescence ~ Diversity + (1 | Composition), PeakInflorescence)
summary(PeakInflorescenceLmer)
anova(PeakInflorescenceLmer)
MuMIn::r.squaredGLMM(PeakInflorescenceLmer)


new_Diversity.PeakInflorescence <- expand.grid(
  Diversity = seq(min(PeakInflorescence$Diversity),
                  max(PeakInflorescence$Diversity),
                  length = 1000)
) %>% as.data.frame()

pred_PeakInflorescenceLmer <- predict(PeakInflorescenceLmer,
                                      newdata = new_Diversity.PeakInflorescence,
                                      re.form = ~0)

ci_line.PeakInflorescenceLmer <- bootMer(
  PeakInflorescenceLmer,
  FUN = function(.) predict(., newdata = new_Diversity.PeakInflorescence, re.form = ~0),
  nsim = 1000
)
ci_line.PeakInflorescenceLmer <- apply(ci_line.PeakInflorescenceLmer$t, 2, function(x) x[order(x)][c(25, 975)])
lb_PeakInflorescenceLmer <- ci_line.PeakInflorescenceLmer[1, ]
ub_PeakInflorescenceLmer <- ci_line.PeakInflorescenceLmer[2, ]

predict.PeakInflorescenceLmer <- cbind(new_Diversity.PeakInflorescence,
                                       pred_PeakInflorescenceLmer,
                                       lb_PeakInflorescenceLmer,
                                       ub_PeakInflorescenceLmer)

# P, R2
anova_PeakInflorescenceLmer <- anova(PeakInflorescenceLmer)
P_PeakInflorescence <- anova_PeakInflorescenceLmer["Diversity", "Pr(>F)"]

R2_PeakInflorescence <- MuMIn::r.squaredGLMM(PeakInflorescenceLmer)
R2m_PeakInflorescence <- R2_PeakInflorescence[1, "R2m"]
R2c_PeakInflorescence <- R2_PeakInflorescence[1, "R2c"]

linetype_PeakInflorescence <- ifelse(P_PeakInflorescence <= 0.05, "solid", "dashed")


fig.PeakInflorescenceLmer <- ggplot() +
  geom_point(data = summary_PeakInflorescence,
             aes(x = Diversity, y = meanInflorescence),
             size = 2, colour = "#199b26") +
  geom_errorbar(data = summary_PeakInflorescence,
                aes(x = Diversity,
                    ymin = lowerInflorescence,
                    ymax = upperInflorescence),
                width = 0.2, colour = "#199b26") +
  geom_ribbon(aes(x = predict.PeakInflorescenceLmer$Diversity,
                  ymin = predict.PeakInflorescenceLmer$lb_PeakInflorescenceLmer,
                  ymax = predict.PeakInflorescenceLmer$ub_PeakInflorescenceLmer),
              fill = "#0433ff", alpha = 0.3) +
  geom_line(aes(x = predict.PeakInflorescenceLmer$Diversity,
                y = predict.PeakInflorescenceLmer$pred_PeakInflorescenceLmer),
            colour = "#0433ff", size = 1,
            linetype = linetype_PeakInflorescence) +
  xlab("Genotypic diversity") +
  ylab("Peak inflorescence number") +
  theme_bw() +
  theme(axis.title = element_text(size = 9),
  ...
  annotate("text", x = 2.2, y = max(summary_PeakInflorescence$meanInflorescence) * 1.09,
           label = sprintf('paste(italic(P), " = %.3f")', P_PeakInflorescence),
           parse = TRUE, size = 2.5) +
  annotate("text", x = 2.2, y = max(summary_PeakInflorescence$meanInflorescence) * 1.05,
           label = sprintf('paste(italic(R)[M]^2, " = %.3f")', R2m_PeakInflorescence),
           parse = TRUE, size = 2.5) +
  annotate("text", x = 2.2, y = max(summary_PeakInflorescence$meanInflorescence) * 1.01,
           label = sprintf('paste(italic(R)[C]^2, " = %.3f")', R2c_PeakInflorescence),
           parse = TRUE, size = 2.5)
fig.PeakInflorescenceLmer
...
anova(PeakInflorescenceDayLmer)
...
anova(PeakMeanHeightLmer)
MuMIn::r.squaredGLMM(PeakMeanHeightLmer)



new_Diversity.PeakMeanHeight <- expand.grid(
   Diversity = seq(min(PeakMeanHeight$Diversity),
                   max(PeakMeanHeight$Diversity),
                   length = 1000)
 ) %>% as.data.frame()

pred_PeakMeanHeightLmer <- predict(PeakMeanHeightLmer,
                                   newdata = new_Diversity.PeakMeanHeight,
                                   re.form = ~0)

ci_line.PeakMeanHeightLmer <- bootMer(
  PeakMeanHeightLmer,
  FUN = function(.) predict(., newdata = new_Diversity.PeakMeanHeight, re.form = ~0),
  nsim = 1000
)
...
predict.PeakMeanHeightLmer <- cbind(new_Diversity.PeakMeanHeight,
                                    pred_PeakMeanHeightLmer,
                                    lb_PeakMeanHeightLmer,
                                    ub_PeakMeanHeightLmer)

anova_PeakMeanHeightLmer <- anova(PeakMeanHeightLmer)
P_PeakMeanHeight <- anova_PeakMeanHeightLmer["Diversity", "Pr(>F)"]

R2_PeakMeanHeight <- MuMIn::r.squaredGLMM(PeakMeanHeightLmer)
R2m_PeakMeanHeight <- R2_PeakMeanHeight[1, "R2m"]
R2c_PeakMeanHeight <- R2_PeakMeanHeight[1, "R2c"]

linetype_PeakMeanHeight <- ifelse(P_PeakMeanHeight <= 0.05, "solid", "dashed")

fig.PeakMeanHeightLmer <- ggplot() +
  geom_point(data = summary_PeakMeanHeight,
             aes(x = Diversity, y = meanMeanHeight),
             size = 2, colour = "#199b26") +
  geom_errorbar(data = summary_PeakMeanHeight,
                aes(x = Diversity,
                    ymin = lowerMeanHeight,
                    ymax = upperMeanHeight),
                width = 0.2, colour = "#199b26") +
  geom_ribbon(aes(x = predict.PeakMeanHeightLmer$Diversity,
                  ymin = predict.PeakMeanHeightLmer$lb_PeakMeanHeightLmer,
                  ymax = predict.PeakMeanHeightLmer$ub_PeakMeanHeightLmer),
              fill = "#0433ff", alpha = 0.3) +
  geom_line(aes(x = predict.PeakMeanHeightLmer$Diversity,
                y = predict.PeakMeanHeightLmer$pred_PeakMeanHeightLmer),
            colour = "#0433ff", size = 1,
            linetype = linetype_PeakMeanHeight) +
  xlab("Genotypic diversity") +
  ylab("Peak height (cm)") +
  theme_bw() +
  theme(axis.title = element_text(size = 9),
        ...
        legend.position = "none") +
  scale_x_continuous(breaks = c(1, 2, 4, 8)) +
  annotate("text", x = 2.2, y = max(summary_PeakMeanHeight$meanMeanHeight) * 1.07,
           label = sprintf('paste(italic(P), " = %.3f")', P_PeakMeanHeight),
           parse = TRUE, size = 2.5) +
  annotate("text", x = 2.2, y = max(summary_PeakMeanHeight$meanMeanHeight) * 1.05,
           label = sprintf('paste(italic(R)[M]^2, " = %.3f")', R2m_PeakMeanHeight),
           parse = TRUE, size = 2.5) +
  annotate("text", x = 2.2, y = max(summary_PeakMeanHeight$meanMeanHeight) * 1.03,
           label = sprintf('paste(italic(R)[C]^2, " = %.3f")', R2c_PeakMeanHeight),
           parse = TRUE, size = 2.5)
...
new_Diversity.PeakRamet <- expand.grid(
  Diversity = seq(min(PeakRamet$Diversity),
                  max(PeakRamet$Diversity),
                  length = 1000)
) %>% as.data.frame()

pred_PeakRametLmer <- predict(PeakRametLmer,
                              newdata = new_Diversity.PeakRamet,
                              re.form = ~0)

ci_line.PeakRametLmer <- bootMer(
  PeakRametLmer,
  FUN = function(.) predict(., newdata = new_Diversity.PeakRamet, re.form = ~0),
  nsim = 1000
)
...
predict.PeakRametLmer <- cbind(new_Diversity.PeakRamet,
                               pred_PeakRametLmer,
                               lb_PeakRametLmer,
                               ub_PeakRametLmer)

anova_PeakRametLmer <- anova(PeakRametLmer)
P_PeakRamet <- anova_PeakRametLmer["Diversity", "Pr(>F)"]

R2_PeakRamet <- MuMIn::r.squaredGLMM(PeakRametLmer)
R2m_PeakRamet <- R2_PeakRamet[1, "R2m"]
R2c_PeakRamet <- R2_PeakRamet[1, "R2c"]

linetype_PeakRamet <- ifelse(P_PeakRamet <= 0.05, "solid", "dashed")

fig.PeakRametLmer <- ggplot() +
  geom_point(data = summary_PeakRamet,
             aes(x = Diversity, y = meanRamet),
             size = 2, colour = "#199b26") +
  geom_errorbar(data = summary_PeakRamet,
                aes(x = Diversity,
                    ymin = lowerRamet,
                    ymax = upperRamet),
                width = 0.2, colour = "#199b26") +
  geom_ribbon(aes(x = predict.PeakRametLmer$Diversity,
                  ymin = predict.PeakRametLmer$lb_PeakRametLmer,
                  ymax = predict.PeakRametLmer$ub_PeakRametLmer),
              fill = "#0433ff", alpha = 0.3) +
  geom_line(aes(x = predict.PeakRametLmer$Diversity,
                y = predict.PeakRametLmer$pred_PeakRametLmer),
            colour = "#0433ff", size = 1,
            linetype = linetype_PeakRamet) +
  xlab("Genotypic diversity") +
  ylab("Peak ramet number") +
  theme_bw() +
  theme(axis.title = element_text(size = 9),
        axis.text  = element_text(size = 9, colour = "black"),
        panel.grid = element_blank(),
        legend.position = "none") +
  scale_x_continuous(breaks = c(1, 2, 4, 8)) +
  annotate("text", x = 2.2, y = max(summary_PeakRamet$meanRamet) * 1.08,
           label = sprintf('paste(italic(P), " < 0.001")', P_PeakRamet),
           parse = TRUE, size = 2.5) +
  annotate("text", x = 2.2, y = max(summary_PeakRamet$meanRamet) * 1.04,
           label = sprintf('paste(italic(R)[M]^2, " = %.3f")', R2m_PeakRamet),
           parse = TRUE, size = 2.5) +
  annotate("text", x = 2.2, y = max(summary_PeakRamet$meanRamet) * 1.0,
           label = sprintf('paste(italic(R)[C]^2, " = %.3f")', R2c_PeakRamet),
           parse = TRUE, size = 2.5)
fig.PeakRametLmer
...
##  ~ combine peak plot  ----
##~~~~~~~~~~~~~~~~~~~~~~
fig.PeakMagnitude <- ggpubr::ggarrange(
  fig.PeakMeanHeightLmer,
  fig.PeakRametLmer,
  fig.PeakInflorescenceLmer,
  ncol = 3, nrow = 1,
...
fig.PeakMagnitude
ggsave("Result/fig.PeakMagnitude20251123.pdf", fig.PeakMagnitude, width = 18, height = 6, units = "cm")
...
PlotDataScale <- PlotData%>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK','MBC','B_chao','B_shannon','B_PC1','F_chao','F_shannon','F_PC1','slope.Ramet','slope.Inflorescence','slope.Inflorescence.Ramet','elevation.Ramet','elevation.Inflorescence','elevation.Inflorescence.Ramet'), ~(scale(.) %>% as.vector))



####slope: Inflorescence ~ MeanHeight####
detach("package:lmerTest", unload = TRUE)

slope.Inflorescence.soil.lmer <- lmer(
  slope.Inflorescence ~  SOC + TN + NH4 + NO3 + AP + AK + 
    MBC + B_chao + B_shannon + B_PC1 + F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(slope.Inflorescence.soil.lmer)
stepba(slope.Inflorescence.soil.lmer)

library(lmerTest)

slope.Inflorescence.soil.final <- lmer(
  slope.Inflorescence ~ AP + F_shannon + (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(slope.Inflorescence.soil.final)

slope.Inflorescence.soil.Summary <- as.data.frame(coef(summary(slope.Inflorescence.soil.final)))
slope.Inflorescence.soil.CI      <- confint(slope.Inflorescence.soil.final)[-c(1, 2), ]
slope.Inflorescence.soil         <- as.data.frame(cbind(slope.Inflorescence.soil.Summary,
                                                        slope.Inflorescence.soil.CI))
slope.Inflorescence.soil$Variable <- row.names(slope.Inflorescence.soil)

slope.Inflorescence.soil <- slope.Inflorescence.soil %>%
  ...
  dplyr::filter(Variable != "(Intercept)") %>%
  dplyr::mutate(
    Colour = ifelse(lower > 0 & upper > 0, 1,
                    ifelse(lower < 0 & upper < 0, -1, 0)),
    Title      = "The scaling exponent",
    Allometry  = "Inflorescence-height"
  )

####intercept: Inflorescence ~ MeanHeight####
detach("package:lmerTest", unload = TRUE)

elevation.Inflorescence.soil.lmer <- lmer(
  elevation.Inflorescence ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 + F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(elevation.Inflorescence.soil.lmer)
stepba(elevation.Inflorescence.soil.lmer)

library(lmerTest)
elevation.Inflorescence.soil.final <- lmer(
  elevation.Inflorescence ~ AP + F_shannon + (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(elevation.Inflorescence.soil.final)

elevation.Inflorescence.soil.Summary <- as.data.frame(coef(summary(elevation.Inflorescence.soil.final)))
elevation.Inflorescence.soil.CI      <- confint(elevation.Inflorescence.soil.final)[-c(1, 2), ]
elevation.Inflorescence.soil         <- as.data.frame(cbind(elevation.Inflorescence.soil.Summary,
                                                            elevation.Inflorescence.soil.CI))
elevation.Inflorescence.soil$Variable <- row.names(elevation.Inflorescence.soil)

elevation.Inflorescence.soil <- elevation.Inflorescence.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = '2.5 %',
                upper = '97.5 %',
...
####slope: Inflorescence ~ Ramet####


detach("package:lmerTest", unload = TRUE)

slope.Inflorescence.Ramet.soil.lmer <- lmer(
  slope.Inflorescence.Ramet ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 + F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(slope.Inflorescence.Ramet.soil.lmer)
stepba(slope.Inflorescence.Ramet.soil.lmer)

library(lmerTest)
slope.Inflorescence.Ramet.soil.final <- lmer(
  slope.Inflorescence.Ramet ~ F_chao + (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(slope.Inflorescence.Ramet.soil.final)

slope.Inflorescence.Ramet.soil.Summary <- as.data.frame(coef(summary(slope.Inflorescence.Ramet.soil.final)))
slope.Inflorescence.Ramet.soil.CI      <- confint(slope.Inflorescence.Ramet.soil.final)[-c(1, 2), ]
slope.Inflorescence.Ramet.soil         <- as.data.frame(cbind(slope.Inflorescence.Ramet.soil.Summary,
                                                              slope.Inflorescence.Ramet.soil.CI))
slope.Inflorescence.Ramet.soil$Variable <- row.names(slope.Inflorescence.Ramet.soil)

slope.Inflorescence.Ramet.soil <- slope.Inflorescence.Ramet.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = '2.5 %',
                upper = '97.5 %',
  ...
  dplyr::mutate(
    Colour    = ifelse(lower > 0 & upper > 0, 1,
                       ifelse(lower < 0 & upper < 0, -1, 0)),
    Title     = "The scaling exponent",
    Allometry = "Inflorescence-ramet")


####intercept: Inflorescence ~ Ramet####
detach("package:lmerTest", unload = TRUE)

elevation.Inflorescence.Ramet.soil.lmer <- lmer(
  elevation.Inflorescence.Ramet ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 + F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(elevation.Inflorescence.Ramet.soil.lmer)
stepba(elevation.Inflorescence.Ramet.soil.lmer)

library(lmerTest)
elevation.Inflorescence.Ramet.soil.final <- lmer(
  elevation.Inflorescence.Ramet ~ F_chao   + (1 | Composition),
  PlotDataScale,
  na.action = "na.fail"
)
summary(elevation.Inflorescence.Ramet.soil.final)

elevation.Inflorescence.Ramet.soil.Summary <- as.data.frame(coef(summary(elevation.Inflorescence.Ramet.soil.final)))
elevation.Inflorescence.Ramet.soil.CI      <- confint(elevation.Inflorescence.Ramet.soil.final)[-c(1, 2), ]
elevation.Inflorescence.Ramet.soil         <- as.data.frame(cbind(elevation.Inflorescence.Ramet.soil.Summary,
                                                                  elevation.Inflorescence.Ramet.soil.CI))
elevation.Inflorescence.Ramet.soil$Variable <- row.names(elevation.Inflorescence.Ramet.soil)

elevation.Inflorescence.Ramet.soil <- elevation.Inflorescence.Ramet.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = '2.5 %',
                upper = '97.5 %',
...
####slope: ramet~height####
detach("package:lmerTest", unload=TRUE) 
slope.Ramet.soil.lmer <-lmer(slope.Ramet~ SOC + TN + NH4 + NO3 + AP + AK + MBC + B_chao + B_shannon + B_PC1 + F_chao + F_shannon + F_PC1+(1|Composition),PlotDataScale, na.action = "na.fail")
...
slope.Ramet.soil <- slope.Ramet.soil %>%
  ...
  mutate(Colour = ifelse(lower > 0 & upper > 0, 1,
                         ifelse(lower < 0 & upper < 0, -1, 0)))%>%
  mutate(Title='The scaling exponent',Allometry = 'Ramet-height')
slope.Ramet.soil




####intercept: ramet~height####

detach("package:lmerTest", unload=TRUE) 
elevation.Ramet.soil.lmer <-lmer(elevation.Ramet~ SOC + TN + NH4 + NO3 + AP + AK + MBC + B_chao + B_shannon + B_PC1 + F_chao + F_shannon + F_PC1+(1|Composition),PlotDataScale, na.action = "na.fail")
...
elevation.Ramet.soil <- elevation.Ramet.soil %>%
  ...
  mutate(Colour = ifelse(lower > 0 & upper > 0, 1,
                         ifelse(lower < 0 & upper < 0, -1, 0)))%>%
  mutate(Title='The intercept',Allometry = 'Ramet-height')
elevation.Ramet.soil

##---------------- combine allometry~soil results ----------------
Allometry.Soil <- rbind(slope.Ramet.soil,
                        elevation.Ramet.soil,
                        slope.Inflorescence.soil,
                        elevation.Inflorescence.soil,
                        slope.Inflorescence.Ramet.soil,
                        elevation.Inflorescence.Ramet.soil) %>%
  mutate(Variable = plyr::mapvalues(Variable,
                                    c("NO3","AP","F_shannon",'F_chao'),
                                    c("Nitrate","Available phosphorus","Fungal Shannon diversity",'Fungal species richness')))

Allometry.Soil$Variable <- factor(Allometry.Soil$Variable,
                               levels = c("Nitrate","Available phosphorus","Fungal Shannon diversity",'Fungal species richness'))

Allometry.Soil$Allometry <- factor(Allometry.Soil$Allometry,
                                      levels = c("Ramet-height",
                                                 "Inflorescence-height",
                                                 "Inflorescence-ramet"))
Allometry.Soil$Title <- factor(Allometry.Soil$Title,
                               levels = c("The scaling exponent","The intercept"))
Allometry.Soil$Colour <- factor(Allometry.Soil$Colour,
                                levels = c(-1, 0, 1))



##---------------- plot ----------------
colors2 <- c('#199b26','#808080','#0433ff')  

Plot.Allometry.Soil <- ggplot(data = Allometry.Soil,
                              aes(x = Variable, y = Estimate, colour = Colour)) +
  facet_grid(Title ~ Allometry, scales = "free_x") +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0, size = 1) +
  geom_point(size = 2, shape = 19) +
  geom_hline(aes(yintercept = 0), colour = "gray", linetype = "dashed") +
  scale_colour_manual(values = c('#199b26','#808080','#0433ff'), labels = c("Negative","Non-significant","Positive"), name = NULL) +
  theme_bw() +
...
#ggsave('Result/Plot.Allometry.Soil20251124.pdf', width =15, height =10, units = 'cm')
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#### THE INFLUENCE OF SOIL ON THE TIMING OF DEVELOPMENTAL PEAKS ----
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

##---------------- Peak height day ----------------
Timing.PeakHeight <- PeakMeanHeight %>%
  dplyr::select(Plot, Composition, Diversity, Day) %>%
  merge(Soil, by = c("Plot","Composition","Diversity"))

Timing.PeakHeight.Scale <- Timing.PeakHeight %>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK',
              'MBC','B_chao','B_shannon','B_PC1',
              'F_chao','F_shannon','F_PC1','Day'),
            ~(scale(.) %>% as.vector))

detach("package:lmerTest", unload = TRUE)

PeakHeight.Day.soil.lmer <- lmer(
  Day ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 +
    F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  Timing.PeakHeight.Scale,
  na.action = "na.fail"
)
summary(PeakHeight.Day.soil.lmer)
stepba(PeakHeight.Day.soil.lmer)

library(lmerTest)


PeakHeight.Day.soil.final <- lmer(
  Day ~ NH4 + (1 | Composition),
  Timing.PeakHeight.Scale,
  na.action = "na.fail"
)
summary(PeakHeight.Day.soil.final)

PeakHeight.Day.soil.Summary <- as.data.frame(coef(summary(PeakHeight.Day.soil.final)))
PeakHeight.Day.soil.CI      <- confint(PeakHeight.Day.soil.final)[-c(1,2),]
PeakHeight.Day.soil         <- as.data.frame(cbind(PeakHeight.Day.soil.Summary,
                                                   PeakHeight.Day.soil.CI))
PeakHeight.Day.soil$Variable <- row.names(PeakHeight.Day.soil)

PeakHeight.Day.soil <- PeakHeight.Day.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = '2.5 %',
                upper = '97.5 %',
  ...
  dplyr::mutate(
    Colour = ifelse(lower > 0 & upper > 0, 1,
                    ifelse(lower < 0 & upper < 0, -1, 0)),
    Trait  = "Peak height day"
  )


##---------------- Peak ramet day ----------------
Timing.PeakRamet <- PeakRamet %>%
  dplyr::select(Plot, Composition, Diversity, Day) %>%
  merge(Soil, by = c("Plot","Composition","Diversity"))

Timing.PeakRamet.Scale <- Timing.PeakRamet %>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK',
              'MBC','B_chao','B_shannon','B_PC1',
              'F_chao','F_shannon','F_PC1','Day'),
            ~(scale(.) %>% as.vector))
...
PeakRamet.Day.soil.lmer <- lmer(
  Day ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 +
    F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  Timing.PeakRamet.Scale,
  na.action = "na.fail"
)
summary(PeakRamet.Day.soil.lmer)
stepba(PeakRamet.Day.soil.lmer)

library(lmerTest)


PeakRamet.Day.soil.final <- lmer(
  Day ~ MBC + (1 | Composition),
  Timing.PeakRamet.Scale,
  na.action = "na.fail"
)
summary(PeakRamet.Day.soil.final)

PeakRamet.Day.soil.Summary <- as.data.frame(coef(summary(PeakRamet.Day.soil.final)))
PeakRamet.Day.soil.CI      <- confint(PeakRamet.Day.soil.final)[-c(1,2),]
PeakRamet.Day.soil         <- as.data.frame(cbind(PeakRamet.Day.soil.Summary,
                                                  PeakRamet.Day.soil.CI))
PeakRamet.Day.soil$Variable <- row.names(PeakRamet.Day.soil)

PeakRamet.Day.soil <- PeakRamet.Day.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = "2.5 %",
                upper = "97.5 %",
  ...
  dplyr::mutate(
    Colour = ifelse(lower > 0 & upper > 0, 1,
                    ifelse(lower < 0 & upper < 0, -1, 0)),
    Trait  = "Peak ramet day"
  )


##---------------- Peak inflorescence day ----------------
Timing.PeakInflorescence <- PeakInflorescence %>%
  dplyr::select(Plot, Composition, Diversity, Day) %>%
  merge(Soil, by = c("Plot","Composition","Diversity"))

Timing.PeakInflorescence.Scale <- Timing.PeakInflorescence %>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK',
              'MBC','B_chao','B_shannon','B_PC1',
              'F_chao','F_shannon','F_PC1','Day'),
            ~(scale(.) %>% as.vector))
...
PeakInflorescence.Day.soil.lmer <- lmer(
  Day ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 +
    F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  Timing.PeakInflorescence.Scale,
  na.action = "na.fail"
)
summary(PeakInflorescence.Day.soil.lmer)
stepba(PeakInflorescence.Day.soil.lmer)

library(lmerTest)


PeakInflorescence.Day.soil.final <- lmer(
  Day ~ AP + (1 | Composition),
  Timing.PeakInflorescence.Scale,
  na.action = "na.fail"
)
summary(PeakInflorescence.Day.soil.final)

PeakInflorescence.Day.soil.Summary <- as.data.frame(coef(summary(PeakInflorescence.Day.soil.final)))
PeakInflorescence.Day.soil.CI      <- confint(PeakInflorescence.Day.soil.final)[-c(1,2),]
PeakInflorescence.Day.soil         <- as.data.frame(cbind(PeakInflorescence.Day.soil.Summary,
                                                          PeakInflorescence.Day.soil.CI))
PeakInflorescence.Day.soil$Variable <- row.names(PeakInflorescence.Day.soil)

PeakInflorescence.Day.soil <- PeakInflorescence.Day.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = "2.5 %",
                upper = "97.5 %",
  ...
  dplyr::mutate(
    Colour = ifelse(lower > 0 & upper > 0, 1,
                    ifelse(lower < 0 & upper < 0, -1, 0)),
    Trait  = "Peak inflorescence day"
  )


##---------------- combine & plot (timing) ----------------
Timing.Soil <- rbind(PeakHeight.Day.soil,
                     PeakRamet.Day.soil,
                     PeakInflorescence.Day.soil) %>%
  mutate(Variable = plyr::mapvalues(Variable,
                                    c('MBC','NH4',"AP"),
                                    c("Microbial biomass carbon","Ammonium","Available phosphorus")))

Timing.Soil$Variable <- factor(Timing.Soil$Variable,
                               levels = c("Ammonium","Available phosphorus","Microbial biomass carbon"))

Timing.Soil$Trait <- factor(Timing.Soil$Trait,
                            levels = c("Peak height day",
                                       "Peak ramet day",
                                       "Peak inflorescence day"))

Timing.Soil$Colour <- factor(Timing.Soil$Colour,
                             levels = c(-1,0,1))

colors2 <- c('#808080','#808080','#808080')

Plot.Timing.Soil <- ggplot(Timing.Soil,
                           aes(x = Variable, y = Estimate, colour = Colour)) +
...
Plot.Timing.Soil
#ggsave("Result/Plot.Timing.Soil.pdf", width = 15, height = 5, units = "cm")
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##                                                                            ~~
####      THE INFLUENCE OF SOIL ON THE MAGNITUDE OF DEVELOPMENTAL PEAKS      ----
##                                                                            ~~
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
##---------------- Peak height (cm) ----------------
Mag.PeakHeight <- PeakMeanHeight %>%
  dplyr::select(Plot, Composition, Diversity, MeanHeight) %>%
  merge(Soil, by = c("Plot","Composition","Diversity"))

Mag.PeakHeight.Scale <- Mag.PeakHeight %>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK',
              'MBC','B_chao','B_shannon','B_PC1',
              'F_chao','F_shannon','F_PC1',
              'MeanHeight'),
...
PeakHeight.Mag.soil.lmer <- lmer(
  MeanHeight ~ SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 +
    F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  Mag.PeakHeight.Scale,
  na.action = "na.fail"
)
summary(PeakHeight.Mag.soil.lmer)
stepba(PeakHeight.Mag.soil.lmer)

library(lmerTest)


PeakHeight.Mag.soil.final <- lmer(
  MeanHeight ~ NO3 + AK + (1 | Composition),
  Mag.PeakHeight.Scale,
  na.action = "na.fail"
)
summary(PeakHeight.Mag.soil.final)

PeakHeight.Mag.soil.Summary <- as.data.frame(coef(summary(PeakHeight.Mag.soil.final)))
PeakHeight.Mag.soil.CI      <- confint(PeakHeight.Mag.soil.final)[-c(1,2),]
PeakHeight.Mag.soil         <- as.data.frame(cbind(PeakHeight.Mag.soil.Summary,
                                                   PeakHeight.Mag.soil.CI))
PeakHeight.Mag.soil$Variable <- row.names(PeakHeight.Mag.soil)

PeakHeight.Mag.soil <- PeakHeight.Mag.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = "2.5 %",
                upper = "97.5 %",
...
##---------------- Peak ramet number ----------------
Mag.PeakRamet <- PeakRamet %>%
  dplyr::select(Plot, Composition, Diversity, Ramet) %>%
  merge(Soil, by = c("Plot","Composition","Diversity"))

Mag.PeakRamet.Scale <- Mag.PeakRamet %>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK',
              'MBC','B_chao','B_shannon','B_PC1',
              'F_chao','F_shannon','F_PC1',
              'Ramet'),
...
PeakRamet.Mag.soil.lmer <- lmer(
  Ramet ~ SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 +
    F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  Mag.PeakRamet.Scale,
...
summary(PeakRamet.Mag.soil.lmer)
stepba(PeakRamet.Mag.soil.lmer)

library(lmerTest)


PeakRamet.Mag.soil.final <- lmer(
  Ramet ~ SOC + AK + MBC + (1 | Composition),
  Mag.PeakRamet.Scale,
  na.action = "na.fail"
)
summary(PeakRamet.Mag.soil.final)

PeakRamet.Mag.soil.Summary <- as.data.frame(coef(summary(PeakRamet.Mag.soil.final)))
PeakRamet.Mag.soil.CI      <- confint(PeakRamet.Mag.soil.final)[-c(1,2),]
PeakRamet.Mag.soil         <- as.data.frame(cbind(PeakRamet.Mag.soil.Summary,
                                                  PeakRamet.Mag.soil.CI))
PeakRamet.Mag.soil$Variable <- row.names(PeakRamet.Mag.soil)

PeakRamet.Mag.soil <- PeakRamet.Mag.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = "2.5 %",
                upper = "97.5 %",
  ...
  dplyr::mutate(
    Colour = ifelse(lower > 0 & upper > 0, 1,
                    ifelse(lower < 0 & upper < 0, -1, 0)),
    Trait  = "Peak ramet number"
  )


##---------------- Peak inflorescence number ----------------
Mag.PeakInflorescence <- PeakInflorescence %>%
  dplyr::select(Plot, Composition, Diversity, Inflorescence) %>%
  merge(Soil, by = c("Plot","Composition","Diversity"))

Mag.PeakInflorescence.Scale <- Mag.PeakInflorescence %>%
  mutate_at(c('Diversity','SOC','TN','NH4','NO3','AP','AK',
              'MBC','B_chao','B_shannon','B_PC1',
              'F_chao','F_shannon','F_PC1',
              'Inflorescence'),
...
PeakInflorescence.Mag.soil.lmer <- lmer(
  Inflorescence ~  SOC + TN + NH4 + NO3 + AP + AK +
    MBC + B_chao + B_shannon + B_PC1 +
    F_chao + F_shannon + F_PC1 +
    (1 | Composition),
  Mag.PeakInflorescence.Scale,
  na.action = "na.fail"
)
summary(PeakInflorescence.Mag.soil.lmer)
stepba(PeakInflorescence.Mag.soil.lmer)

library(lmerTest)


PeakInflorescence.Mag.soil.final <- lmer(
  Inflorescence ~ AK + (1 | Composition),
  Mag.PeakInflorescence.Scale,
  na.action = "na.fail"
)
summary(PeakInflorescence.Mag.soil.final)

PeakInflorescence.Mag.soil.Summary <- as.data.frame(coef(summary(PeakInflorescence.Mag.soil.final)))
PeakInflorescence.Mag.soil.CI      <- confint(PeakInflorescence.Mag.soil.final)[-c(1,2),]
PeakInflorescence.Mag.soil         <- as.data.frame(cbind(PeakInflorescence.Mag.soil.Summary,
                                                          PeakInflorescence.Mag.soil.CI))
PeakInflorescence.Mag.soil$Variable <- row.names(PeakInflorescence.Mag.soil)

PeakInflorescence.Mag.soil <- PeakInflorescence.Mag.soil %>%
  dplyr::select(Variable,
                Estimate,
                lower = "2.5 %",
                upper = "97.5 %",
  ...
  dplyr::mutate(
    Colour = ifelse(lower > 0 & upper > 0, 1,
                    ifelse(lower < 0 & upper < 0, -1, 0)),
    Trait  = "Peak inflorescence number"
  )

unique(Magnitude.Soil$Variable)
##---------------- combine & plot (magnitude) ----------------
Magnitude.Soil <- rbind(PeakHeight.Mag.soil,
                        PeakRamet.Mag.soil,
                        PeakInflorescence.Mag.soil) %>%
  mutate(Variable = plyr::mapvalues(Variable,
                                    c("NO3", "AK" , "SOC", "MBC"),
                                    c("Nitrate","Available potassium", "Soil organic carbon", "Microbial biomass carbon")))

Magnitude.Soil$Variable <- factor(Magnitude.Soil$Variable,
                                  levels = c("Soil organic carbon","Nitrate","Available potassium",  "Microbial biomass carbon"))

Magnitude.Soil$Trait <- factor(Magnitude.Soil$Trait,
                               levels = c("Peak height",
                                          "Peak ramet number",
                                          "Peak inflorescence number"))

Magnitude.Soil$Colour <- factor(Magnitude.Soil$Colour,
                                levels = c(-1,0,1))

colors2 <- c('#199b26','#0433ff','#0433ff')

Plot.Magnitude.Soil <- ggplot(Magnitude.Soil,
                              aes(x = Variable, y = Estimate, colour = Colour)) +
  facet_grid(. ~ Trait, scales = "free_x") +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0, size = 1) +
  geom_point(size = 2, shape = 19) +
...
Plot.Magnitude.Soil
#ggsave("Result/Plot.Magnitude.Soil.pdf", width = 16, height = 5, units = "cm")
...
####~Soil predictors of allometric exponents & intercepts####
#Figure: Plot.Allometry.Soil)


get_anova_table <- function(model, response_name){
  a <- anova(model)
  a <- as.data.frame(a)
...
## anova: allometry–soil final model
tab_slope_Ramet  <- get_anova_table(slope.Ramet.soil.final,
                                    "Scaling exponent (ramet–height)")
tab_int_Ramet    <- get_anova_table(elevation.Ramet.soil.final,
                                    "Intercept (ramet–height)")

tab_slope_InfH   <- get_anova_table(slope.Inflorescence.soil.final,
                                    "Scaling exponent (inflorescence–height)")
tab_int_InfH     <- get_anova_table(elevation.Inflorescence.soil.final,
                                    "Intercept (inflorescence–height)")

tab_slope_InfR   <- get_anova_table(slope.Inflorescence.Ramet.soil.final,
                                    "Scaling exponent (inflorescence–ramet)")
tab_int_InfR     <- get_anova_table(elevation.Inflorescence.Ramet.soil.final,
                                  "Intercept (inflorescence–ramet)")


Allometry_tab <- bind_rows(
  tab_slope_Ramet,
  tab_int_Ramet,
  tab_slope_InfH,
  tab_int_InfH,
...
unique(Allometry_tab$Predictor)

Allometry_tab <- Allometry_tab %>%
  mutate(Predictor  = plyr::mapvalues(Predictor, c("NO3","AP","F_shannon","F_chao"), c("Nitrate","Available phosphorus","Fungal Shannon diversity","Fungal species richness")))%>%
  mutate(
    #Num_df  = round(Num_df, 1),
    Den_df  = round(Den_df, 3),
    F_value = round(F_value, 3),
    P_value = round(P_value, 3)
  )


write.xlsx(Allometry_tab,
           file = "Result/Table_Allometry_Soil_LMM.xlsx",
           rowNames = FALSE)



####~Soil predictors of timing of developmental peaks####
##    (Figure: Plot.Timing.Soil)


tab_PH_day  <- get_anova_table(PeakHeight.Day.soil.final,
                               "Peak height day")
tab_PR_day  <- get_anova_table(PeakRamet.Day.soil.final,
                               "Peak ramet day")
tab_PI_day  <- get_anova_table(PeakInflorescence.Day.soil.final,
                               "Peak inflorescence day")

Timing_tab <- bind_rows(tab_PH_day, tab_PR_day, tab_PI_day) %>%
  mutate(Predictor  = plyr::mapvalues(Predictor, c("NH4","AP","MBC"), c("Ammonium","Available phosphorus","Microbial biomass carbon")))%>%
  mutate(
    #Num_df  = round(Num_df, 1),
    Den_df  = round(Den_df, 3),
    F_value = round(F_value, 3),
...
#write.xlsx(Timing_tab,
           #file = "Result/Table_Timing_Soil_LMM.xlsx",
           #rowNames = FALSE)



####~Soil predictors of magnitude of developmental peaks####
##    (Figure: Plot.Magnitude.Soil)


tab_PH_mag <- get_anova_table(PeakHeight.Mag.soil.final,
                              "Peak height")
tab_PR_mag <- get_anova_table(PeakRamet.Mag.soil.final,
                              "Peak ramet number")
tab_PI_mag <- get_anova_table(PeakInflorescence.Mag.soil.final,
                              "Peak inflorescence number")

Magnitude_tab <- bind_rows(tab_PH_mag, tab_PR_mag, tab_PI_mag) %>%
  mutate(Predictor  = plyr::mapvalues(Predictor, c("SOC","NO3","AK", "MBC"), c("Soil organic carbon","Nitrate","Available potassium","Microbial biomass carbon")))%>%
  mutate(
    #Num_df  = round(Num_df, 1),
    Den_df  = round(Den_df, 3),
    F_value = round(F_value, 3),
...
#write.xlsx(Magnitude_tab,
           #file = "Result/Table_Magnitude_Soil_LMM.xlsx",
           #rowNames = FALSE)
